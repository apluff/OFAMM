...
% TS
handles.CorrWinPixelsTS = 3;
handles.CorrWinFramesTS = 100;
handles.MaxLagTS = 50;
handles.TargetFramesTS = 10;

% Replacement file specifications from OFAMM_v1
FilterSpec = {'*.tif; *.raw; *.mat'};
DialogTitle = 'Select the mask image';
% [FileName,PathName,FilterIndex] = uigetfile(FilterSpec,DialogTitle);
% [PathName, FileName, ExtName] = fileparts([PathName,FileName]);
[PathName, FileName, ExtName] = fileparts(input_path);
...
if handles.ImgSeqLoaded
    ...
    % end
	FstartOF = FstartOF_default;
	FendOF = FendOF_default;
    ...
    if runCLG
        ...
        % end
		CLGparams.alpha = handles.alphaCLG;
        CLGparams.ratio = handles.ratioCLG;
        CLGparams.minWidth = handles.minWidthCLG;
        CLGparams.nOuterFPIterations = handles.nOuterFPIterationsCLG;
        CLGparams.nInnerFPIterations = handles.nInnerFPIterationsCLG;
        CLGparams.nSORIterations = handles.nSORIterationsCLG;
    ...
    if runHS
        ...
        % end
		HSparams.alpha = handles.alphaHS;
        HSparams.iterations = handles.IterationsHS;
    end
...
%             [im1, im2] = normalize_two_consequtive_frames(im1,im2,handles.idxMask);
    ...
    if runTS
        ...
        % end
		TSparams.CorrWinPixels = handles.CorrWinPixelsTS;
        TSparams.CorrWinFrames = handles.CorrWinFramesTS;
        TSparams.MaxLag = handles.MaxLagTS;
        TSparams.TargetFrames = handles.TargetFramesTS;
        tic
    ...
    if SaveOF
        ...
        if saveCLG || saveHS || saveTS
            if isfield(handles,'PathName')
                ...
                handles.SavePathName = output_path; % current folder
            end
        end
